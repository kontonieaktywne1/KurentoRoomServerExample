...
public class JsonRpcUserControl {
  ...
  public void paintSend(Transaction transaction, Request<JsonObject> request,
	      ParticipantRequest participantRequest) {
		  String userName = getStringParam(request, ProtocolElements.ONPAINTSEND_USER_PARAM);
		  String roomName = getStringParam(request, ProtocolElements.ONPAINTSEND_ROOM_PARAM);
		  String coordinates = getStringParam(request, ProtocolElements.ONPAINTSEND_COORDS_PARAM);
		  String windowSize = getStringParam(request, ProtocolElements.ONPAINTSEND_DISPLAY_SIZE_PARAM);
		  String brushSize = getStringParam(request, ProtocolElements.ONPAINTSEND_BRUSH_SIZE_PARAM);
		  String brushColor = getStringParam(request, ProtocolElements.ONPAINTSEND_BRUSH_COLOR_PARAM);
		  String brushType = getStringParam(request, ProtocolElements.ONPAINTSEND_BRUSH_TYPE_PARAM);
		  
		  roomManager.sendPaint(userName, coordinates, windowSize, brushColor,
				  brushSize, brushType, roomName, participantRequest);
	  }
  
  public void recordSession(Transaction transaction, Request<JsonObject> request,
		  ParticipantRequest participantRequest) {
	String record = getStringParam(request, ProtocolElements.RECORD_SESSION_PARAM);
	String room = getStringParam(request, ProtocolElements.RECORD_ROOM_PARAM);
	
	roomManager.recordSession(room, record, participantRequest);
  }
