...
public class NotificationRoomManager {
  ...
  public void sendPaint(String userName, String coordinates, String windowSize, 
		  String brushColor, String brushSize, String brushType, String roomName, ParticipantRequest request) {
	  try {
	      if (!internalManager.getRoomName(request.getParticipantId()).equals(roomName)) {
	        throw new RoomException(Code.ROOM_NOT_FOUND_ERROR_CODE, "Provided room name '" + roomName
	            + "' differs from the participant's room");
	      }
	      notificationRoomHandler.onPaintSend(request, userName, coordinates, windowSize, brushColor,
	    		  brushSize, brushType, roomName, internalManager.getParticipants(roomName), null);
	      if (internalManager.isPublisherRecording(request.getParticipantId())) {
	    	  internalManager.saveCoordToFile(request.getParticipantId(), coordinates, windowSize, 
	    			  brushColor, brushSize, brushType);
	      }
	    } catch (RoomException e) {
	      log.warn("PARTICIPANT {}: Error sending message", e);
	      notificationRoomHandler.onPaintSend(request, null, null, null, null, null, null,
	    		  null, null, e);
	    }
}
  
  public void recordSession(String roomName, String record, ParticipantRequest participantRequest) {
	  boolean recordSession = Boolean.parseBoolean(record);
	  if (recordSession) {
		  if (internalManager.isPublisherRecording(participantRequest.getParticipantId())) {
			  log.warn("PARTICIPANT {}: Session is recordered");
		  } else {
			  Set<UserParticipant> existingParticipant = null;
			  try {
				  existingParticipant = internalManager.recordSession(roomName,
						  participantRequest.getParticipantId());
			  } catch (RoomException e) {
				  log.warn("PARTICIPANT {}: Error recording session", e);
			  }
			  if (existingParticipant != null) {
				  notificationRoomHandler.onSessionRecording(participantRequest, null);
			  }
		  }
	  } else {
		  if (!internalManager.isPublisherRecording(participantRequest.getParticipantId())) {
			  log.warn("PARTICIPANT {}: Session is not recordered");
		  } else {
			  Set<UserParticipant> existingParticipant = null;
			  try {
				  existingParticipant = internalManager.stopRecording(roomName,
						  participantRequest.getParticipantId());
			  } catch (RoomException e) {
				  log.warn("PARTICIPANT {}: Error stop recording", e);
			  }
			  if (existingParticipant != null) {
				  notificationRoomHandler.onStopSessionRecording(participantRequest, null);
			  }
		  }
	  }
	}
